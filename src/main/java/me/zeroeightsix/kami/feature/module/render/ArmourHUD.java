...
public class ArmourHUD extends Module {
    ...
    public Listener<RenderHudEvent> renderListener = new Listener<>(event -> {
        ...
        int iteration = 0;
        int y = MinecraftClient.getInstance().window.getScaledHeight() - 55 - (mc.player.isSubmergedInWater() ? 10 : 0);
        for (ItemStack is : mc.player.inventory.armor) {
            ...
            mc.textRenderer.drawWithShadow(s, x + 19 - 2 - mc.textRenderer.getWidth(s), y + 9, 0xffffff);

            if (damage) {
                float green = ((float) is.getMaxDamage() - (float) is.getDamage()) / (float) is.getMaxDamage();
                float red = 1 - green;
                int dmg = 100 - (int) (red * 100);
                mc.textRenderer.drawWithShadow(dmg + "", x + 8 - mc.textRenderer.getWidth(dmg + "") / 2, y - 11, ColourHolder.toHex((int) (red * 255), (int) (green * 255), 0));
            }
        }
