...
object FriendCommand : Command() {
                            ...
                            createFriendArgument(
                                Function { entry: PlayerListEntry ->
                                    if (Friends.isFriend(entry.profile.name)) {
                                        return@Function FAILED_EXCEPTION.create("That player is already your friend!")
                                    }
                                    null
                                },
                    ...
                    LiteralArgumentBuilder.literal<CommandSource>("remove")
                        .then(
                            createFriendArgument(
                                Function { entry: PlayerListEntry ->
                                    if (!Friends.isFriend(entry.profile.name)) {
                                        return@Function FAILED_EXCEPTION.create("That player isn't your friend!")
                                    }
                                    null
                                },
                                BiFunction { entry: PlayerListEntry, source: KamiCommandSource ->
    ...
    private fun requestIDs(data: String): String? {
        return try {
            ...
            val res = convertStreamToString(input)
            input.close()
            conn.disconnect()
            res
        } catch (e: Exception) {
